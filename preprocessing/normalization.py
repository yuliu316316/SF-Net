...
def init_args():
    ...
    parser.add_argument('-y', '--year', type=int, help='st1c_file,', default=202001)
...
def get_list_of_files(base_dir):
    ...
    for glioma_type in ['HGG', 'LGG']:
        ...
        for p in patients:
            ...
            this_case = [ t1c_file,t2_file, flair_file, seg_file]
...
def get_list_of_files_2020(current_directory, patients, mode="training"):
    ...
    for p in patients:
        ...
        t1c_file = join(patient_directory, p + "_t1ce.nii.gz")
        t2_file = join(patient_directory, p + "_t2.nii.gz")
        flair_file = join(patient_directory, p + "_flair.nii.gz")   #BraTS19_2013_2_1_t2flair_fusion.nii.gz
        # fusion1_file = join(patient_directory, p + "_t2flair_fusion.nii.gz")
        # fusion1_file = join(patient_directory, p + "_t1ceflair_fusion.nii.gz")
        # fusion2_file = join(patient_directory, p + "_t1cet2_fusion.nii.gz")
        if mode == "training":
            seg_file = join(patient_directory, p + "_seg.nii.gz")
            this_case = [t1c_file,t2_file, flair_file,seg_file]
            # this_case = [t1_file, t1c_file,t2_file, flair_file,fusion1_file,fusion2_file, seg_file]
        else:
            this_case = [t1c_file,t2_file, flair_file]
            # this_case = [t1_file, t1c_file,t2_file, flair_file,fusion1_file,fusion2_file]
        assert all((isfile(i) for i in this_case)), "some file is missing for patient %s; make sure the following " \
                                                    "files are there: %s" % (p, str(this_case))
...
def load_and_preprocess(case, patient_name, output_folder):
    ...
    imgs_npy1=imgs_npy[:-1]
    # print(imgs_npy1.shape)
    ...
    imgs_npy2=np.concatenate((imgs_npy,imgs_npy1),axis=0)
    print("5d",imgs_npy2.shape)
    # now save as npy
    #if not os.path.exists(output_folder):
    #    os.mkdir(output_folder)
    np.save(join(output_folder, patient_name + ".npy"), imgs_npy2)
...
if __name__ == "__main__":
    ...
    if args.year == 2018:
        ...
        data_file_path = "data/MICCAI_BraTS_2019_Data_Training"
    ...
    if args.year == 2020:
        ...
        # HGG = name_mapping.loc[name_mapping.Grade == "HGG", "BraTS_2019_subject_ID"].tolist()
        # LGG = name_mapping.loc[name_mapping.Grade == "LGG", "BraTS_2019_subject_ID"].tolist()
        patients = HGG + LGG
    ...
    if args.year == 202001 or args.year == 202002:
        ...
        if args.year == 202001:
            data_file_path = "data/MICCAI_BraTS_2019_Data_Validation"
        ...
        npy_normalized_folder = join(data_file_path, "npy")
        name_mapping = pd.read_csv(mapping_file_path)
        patients = name_mapping["BraTS_2019_subject_ID"].tolist()#BraTS20ID
        # patients =["BraTS19_CBICA_AAM_1"]
